...
Browser (Vibe tab)
   ...
   │    └─ Returns {run_id}         └─ subprocess: vibe --workdir … --prompt …
   │
...
| Service | `openclaw_control/service.py` | `start_vibe_run` / `get_vibe_run`; long-running subprocess in daemon thread |
...
| Config | `openclaw_control/config.py` | `OPENCLAW_VIBE_WORKDIR` — default working directory |

---

## Permission Model

Execution of the `vibe` CLI is **always gated by an explicit human approval step**:
...
2. **Approve** — Clicking **▶ Approve & Execute** surfaces a yellow warning banner showing the exact command that will run:
   ```
   vibe --workdir /opt/openclaw-crypto --prompt "<prompt text>"
   ```
3. **Confirm** — Only after clicking **✅ Confirm & Execute** does the backend spawn the subprocess.
...
Start a Vibe run after the user has approved the workdir and prompt.
...
```
 ...
 │  │  vibe --workdir /opt/openclaw-crypto --prompt "Add stop-loss…" │   │
...
- The subprocess timeout is 900 seconds. Long-running Vibe invocations block one daemon thread per run; in-flight runs survive a server restart only if the process is still alive.
- No shell expansion is used: `workdir` and `prompt` are passed as separate argv elements to `subprocess.run`, preventing shell injection.
...
- [ ] Click **▶ Approve & Execute** — yellow approval banner appears with exact command
