...
Supported transports (tried in order):
  1. HTTP — when VIBE_API_BASE is set (e.g. http://127.0.0.1:7000).
  2. CLI  — when /usr/local/bin/vibe (or VIBE_CLI_PATH) is present.
  If neither is available the call is rejected with exit_code=1.
    ...
    **extra_params:
    ...
    if api_base:
        ...
        result = {
            ...
            "stdout": "",
            "stderr": (
                "No Vibe transport available: set VIBE_API_BASE or install the "
                "Vibe CLI at /usr/local/bin/vibe (or override with VIBE_CLI_PATH)."
            ),
        }
